...
function trendSeriesSvg(key=S.brainTrendDomain){
  ...
  const pts=rows.map((x,i)=>{const v=trendValue(x,key);const xx=rows.length===1?w/2:p+i*((w-2*p)/(rows.length-1));const yy=h-p-(v*(h-2*p)/100);return [xx,yy,v]});
  const area=pts.length>1?'<path d="M '+pts[0][0]+' '+(h-p)+' L '+pts.map(x=>x[0]+' '+x[1]).join(' L ')+' L '+pts[pts.length-1][0]+' '+(h-p)+' Z" class="overall-area"/>':'';
  const line=pts.length>1?'<polyline points="'+pts.map(x=>x[0]+','+x[1]).join(' ')+'" class="overall-line"/>':'';
  return '<svg class="overall-trend" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="기능 변화 추이">'+area+line+pts.map((x,i)=>'<circle cx="'+x[0]+'" cy="'+x[1]+'" r="'+(i===pts.length-1?6:3.5)+'" class="'+(i===pts.length-1?'latest':'')+'"/>').join('')+'</svg>';
}
...
const MONITORING_UI_LEVELS={
  stable:{key:'stable',label:'안정',icon:'circle-check',headline:'지금은 내 평소 범위 안에 있습니다',action:'현재 기록을 이어가세요'},
...
async function demoAnimateEvidence(alive){
  await demoGo('evidence-proof',280,{title:'이 판단에는 실제 연구 근거를 사용합니다',position:'bottom',hold:420});
  if(!alive())return false;
  ...
  try{
    ...
    if(!await show('factors','관찰 항목마다 근거를 함께 확인합니다',1350))return false;
    if(!await show('sources','논문과 가이드라인 원문까지 확인할 수 있습니다',1250))return false;
  }catch(err){
...
async function demoCycleBrain(alive){
  for(const key of ['memory','executive','language','spatial','daily']){
    if(!alive())return false;
    S.brainFocus=key;
    if(['memory','language'].includes(key))S.brainView='side';
    save();render();demoScrollTop();
    await demoWait(680);
  }
  return alive();
}
async function demoCycleTrend(alive){
  S.brainRange='year';S.brainTrendDomain='overall';save();render();demoScrollTop();await demoWait(850);
  for(const key of ['memory','executive','language','spatial','overall']){
    if(!alive())return false;
    S.brainTrendDomain=key;save();render();demoScrollTop();
    await demoWait(620);
  }
  return alive();
}
...
async function runRealAppTour(prepared=false){
  ...
  try{
    ...
    await demoGo('research-engine',4400,{title:'새 연구는 바로 적용하지 않고 기존 기준과 먼저 비교합니다',position:'bottom',hold:1200});if(!alive())return;
    ...
    // 변화가 쌓이는 과정을 보여주는 장면.
    await demoGo('baseline',1300,{title:'14일 동안 내 평소를 먼저 만듭니다',position:'bottom',hold:900});if(!alive())return;
    ...
    // 결론은 다시 충분히 멈춘다.
    await demoGo('monitoring-status',3300,{title:'변화가 커지면 상담이나 보호자 연결로 바로 이어집니다',position:'bottom',hold:1100});if(!alive())return;
    S.mode='care';save();await demoGo('caregiver-home',2600,{title:'필요하면 보호자도 같은 결과를 함께 확인합니다',position:'bottom',hold:1000});if(!alive())return;
    S.mode='self';save();await demoGo('monitoring-status',3600,{title:'내 평소와 비교하고, 필요한 다음 행동까지 연결합니다',position:'bottom',hold:1100});if(!alive())return;
  }finally{
    if(!alive())return;
...
page['monitoring-status']=()=>{
  ...
  return wrap(
    ...
    '<h1 class="page-title">'+(ready?status.headline:'아직 내 평소를 만드는 중입니다')+'</h1>'+
...
page['brain-trends']=()=>{
  ...
  return wrap(
    '<div class="eyebrow">시간에 따른 변화</div>'+
    '<div class="trend-title-row"><h1 class="page-title">'+(key==='overall'?'전체 흐름':'“'+meta[1]+'”')+'을<br>시간으로 이어서 봅니다</h1><span class="trend-delta '+(delta===null?'neutral':delta<0?'down':'up')+'">'+(delta===null?'기록 중':(delta>0?'+':'')+delta+'p')+'</span></div>'+
