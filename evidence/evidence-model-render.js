...
function render(m){
  ...
  if(onboardingItems.length||baselineTasks.length){
    ...
    questionHtml='<div class="card mb-4" id="question-registry"><div class="card-header"><div><div class="text-uppercase text-secondary small">QUESTION GOVERNANCE</div><h2 class="card-title mt-1">앱이 실제 묻는 항목 ↔ 근거 1:1 매핑</h2><div class="text-secondary small mt-1">'+esc(onboarding.principle||'모든 조사 항목은 근거 역할과 출처를 등록합니다.')+'</div></div></div>'
      +'<div class="table-responsive"><table class="table card-table table-vcenter"><thead><tr><th>앱 조사 항목</th><th>역할</th><th>근거</th><th>앱에서의 사용</th></tr></thead><tbody>'
      +onboardingItems.map(function(x){return '<tr><td><strong>'+esc(x.label)+'</strong><div class="text-secondary small">'+esc(x.appField||'')+'</div></td><td><span class="badge bg-blue-lt">'+esc(roleLabel[x.role]||x.role||'')+'</span></td><td>'+esc(x.evidence||'')+'<div class="text-secondary small">'+esc(x.source||'')+'</div></td><td>'+esc(x.use||'')+(x.sourceUrl?'<div><a class="small" target="_blank" rel="noopener" href="'+esc(x.sourceUrl)+'">원문 ↗</a></div>':'')+'</td></tr>'}).join('')
      +'</tbody></table></div>'
      +(baselineTasks.length?'<div class="card-body border-top"><h3 class="card-title mb-3">KIMSE 초기 baseline 과제</h3><div class="row g-3">'+baselineTasks.map(function(x){return '<div class="col-md-6"><div class="card h-100"><div class="card-body"><div class="d-flex justify-content-between gap-2"><strong>'+esc(x.label)+'</strong><span class="badge bg-green-lt">'+esc(roleLabel[x.role]||x.role||'')+'</span></div><div class="text-secondary small mt-2">'+esc(x.evidence||'')+'</div><p class="mt-2 mb-2">'+esc(x.implementation||'')+'</p>'+(x.sourceUrl?'<a class="small" target="_blank" rel="noopener" href="'+esc(x.sourceUrl)+'">근거 원문 ↗</a>':'')+'</div></div></div>'}).join('')+'</div><div class="alert alert-warning mt-3 mb-0"><strong>중요</strong><br>KIMSE 자체 문항은 검증도구의 임상 점수·cutoff·민감도·특이도로 환산하지 않습니다.</div></div>':'')
      +'</div>';
  }
